...
class ShardingGroupCustomCallPattern
  ...
  mlir::LogicalResult matchAndRewrite(
    ...
    rewriter.replaceOpWithNewOp<ShardingGroupOp>(
        op, adaptor.getInputs().front(), shardingGroupId.value().getInt());

    return mlir::success();
  }
};

// Converts a `CustomCallOp` with target name xla.sdy.ShardingGroup into a
// `ShardingGroupOp`.
class ConvertShardingGroupCustomCallsPass
 ...
 public:
  ...
  void runOnOperation() final {
    ...
    target.addLegalOp<ShardingGroupOp>();
  ...
  StringRef getDescription() const override {
    return "Converts a CustomCallOp with target name ShardingGroup into a "
           "ShardingGroupOp.";
  }
};
